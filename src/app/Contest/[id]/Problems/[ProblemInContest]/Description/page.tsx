...
import AddToCollectionModal from "@/app/Problems/components/AddToCollectionModal";
...
export default function DescriptionTab() {
  ...
  const { data: checkData } = useCheckFavoriteQuery({ problemId: ProblemInContest }, { skip: !ProblemInContest || !userData });
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      const res = await toggleFavorite(ProblemInContest).unwrap();
  ...
  return (
    ...
    >
      ...
      <div className="h-8" />
      <Discussion problemId={ProblemInContest} currentUserId={userData?.userId} />

      {/* MODALS */}
      <AddToCollectionModal
        isOpen={isAddOpen}
        onOpenChange={onOpenChangeAdd}
        problemId={ProblemInContest}
      />
    </div>
  );
}
