...
const { data: semestersData } = useGetALLSemestersQuery();
  ...
  // Hàm format hiển thị (ví dụ: FALL 2025 → "FALL 2025")
...
const openUpdateTeacherForClass = (
  ...
  console.log("aaaaaaaaaa",classes);
  ...
  return (
    <div className="flex flex-col h-full gap-8 p-2">
      ...
      <DropdownMenu 
        ...
        // onAction={(key) => console.log("Selected semester key:", key)} // xử lý khi chọn
      >
        {semesters.length > 0 ? (
          semesters.map((semester: SemesterItem) => (
            <DropdownItem 
              key={semester.semesterId || semester.code} // dùng id hoặc semesterCode làm key
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-start gap-2">
                      <Chip
                        variant="flat"
                        size="sm"
                        className="font-black bg-slate-100 dark:bg-gray-800 text-slate-500 dark:text-gray-400 uppercase italic text-[9px] h-5 relative z-20"
                      >
  ...
  <div className="relative z-20 flex flex-1 h-[34px] rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111c35]">
    ...
    {/* Nền màu vàng bên phải (gạch chéo) */}
    <div 
