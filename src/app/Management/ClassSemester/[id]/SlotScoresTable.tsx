...
export default function SlotScoresTable({ classId, slot }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full mt-4">
      {/* Filter Header */}
      <div className="flex justify-between items-center bg-slate-50 dark:bg-white/5 p-3 rounded-2xl border border-divider">
        <div className="font-[1000] uppercase text-xs text-slate-500 tracking-[0.2em] pl-3 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
          {scores.length} Students Participated
        </div>
        <Input
      ...
      {/* Scores Table */}
      <div className="w-full bg-white dark:bg-[#111c35] rounded-[2rem] border border-divider overflow-hidden custom-scrollbar">
        <Table 
        ...
        >
          <TableHeader>
            {[
              <TableColumn key="student" className="sticky left-0 z-20 bg-slate-100 dark:bg-[#1A2332]">STUDENT</TableColumn>,
              <TableColumn key="total" className="text-center font-black text-[#FF5C00] bg-orange-50 dark:bg-orange-900/20">TOTAL SCORE</TableColumn>,
              ...problemColumns.map((p: any, idx: number) => (
                <TableColumn key={`col-${p.problemId}`} className="text-center min-w-[140px] max-w-[200px]">
                  <div className="flex flex-col items-center gap-1">
                    <Chip size="sm" variant="flat" className="bg-blue-100 text-blue-600 dark:bg-blue-900/30 text-[10px] font-black h-5 px-2">
                      Prob {p.ordinal ?? idx + 1}
                    </Chip>
              ...
              <TableColumn key="actions" className="text-right pr-6">ACTIONS</TableColumn>
            ]}
          </TableHeader>
          <TableBody emptyContent="No score data available for any students in this slot.">
            {filteredScores.map((student: any) => (
              <TableRow key={student.userId} className="hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer">
                {[
                  ...
                  <TableCell key="total-score" className="text-center bg-orange-50/30 dark:bg-orange-900/10">
                    ...
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                      {student.solvedCount}/{problemColumns.length} Solved
                    </div>
                  </TableCell>,
                  ...
                  ...problemColumns.map((p: any) => {
                    ...
                    return (
                      <TableCell key={`cell-${p.problemId}`} className="text-center">
                        ...
                        <div className="text-[10px] text-slate-400 font-bold mt-0.5 flex items-center justify-center gap-1">
                          <span className={`w-1.5 h-1.5 rounded-full ${isAC ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
                          {problemScore.attempts} attempt(s)
                        </div>
                      </TableCell>
                    );
                  }),
                  ...
                  <TableCell key="actions" className="pr-6">
                    <div className="flex justify-end">
                      <Button
                        ...
                        onPress={() => setSelectedUser({ id: student.userId, name: student.displayName || "Student" })}
