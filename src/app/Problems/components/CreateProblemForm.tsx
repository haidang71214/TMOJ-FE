...
export default function CreateProblemForm() {
  ...
  if (!userData?.userId) {
    addToast({ 
      ...
      description: t('problem_create.user_not_loaded') || "User not loaded yet", 
      color: "danger" 
    });
    return;
  }
  ...
  if (!form.slug || !form.title) {
    addToast({ 
      title: t('common.error') || "Error", 
      description: t('problem_create.slug_title_required') || "Slug and Title are required", 
      color: "danger" 
    });
    return;
  }
  ...
  const hasFile = !!statementFile;

  if (hasDescription && hasFile) {
    addToast({ 
      title: t('common.error'), 
      description: language === 'vi' ? "Chỉ được chọn nhập Markdown hoặc tải file, không thể dùng cả hai." : "Choose either Markdown OR file upload.", 
      color: "danger" 
    });
    return;
  }
  ...
  if (!hasDescription && !hasFile) {
    addToast({ 
      title: t('common.error'), 
      description: language === 'vi' ? "Vui lòng nhập mô tả hoặc tải file đề bài." : "Description or Statement file is required.", 
      color: "danger" 
    });
    return;
  }
  ...
  try {
    ...
    addToast({
      title: t('problem_create.create_failed') || "Create Failed",
      description: err?.data?.data?.message || t('problem_create.failed_draft'),
      color: "danger"
    });
  }
};
  ...
  return (
    ...
    label={
      <div className="flex items-center gap-1">
        {t('problem_create.problem_mode') || "Problem Mode"}
        <RequiredStar rules={[t('common.required_field') || "Required field"]} />
      </div>
    }
