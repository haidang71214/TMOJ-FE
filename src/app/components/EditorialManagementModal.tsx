...
import { BookOpen, Save, Trash2, Eye, Edit3, Loader2, Sparkles, Settings2 } from "lucide-react";
...
import { useGenerateAiEditorialMutation } from "@/store/queries/ai";
...
export default function EditorialManagementModal({
  ...
  const existingEditorial = editorialData?.[0];
  ...
  const [generateAiEditorial, { isLoading: isGenerating }] = useGenerateAiEditorialMutation();

  const handleGenerateAiDraft = async () => {
    if (!problemId) return;

    try {
      const response = await generateAiEditorial({
        problemId,
        languageCode: "en",
        styleCode: "educational",
        targetAudienceCode: "student",
        ...aiOptions
      }).unwrap();

      if (response.data.succeeded) {
        setContent(response.data.data.contentMd);
        addToast({
          title: "AI Draft Generated",
          description: "AI has created a suggested editorial for you.",
          color: "success"
        });
      } else {
        addToast({
          title: "AI Generation Failed",
          description: response.data.message || "Failed to generate AI editorial",
          color: "danger"
        });
      }
    } catch (error: any) {
          ...
          <>
            ...
            <ModalFooter className="border-t dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
              ...
              <Popover placement="top-end" backdrop="blur">
                ...
                <PopoverContent className="p-4 w-64 bg-white dark:bg-[#0f172a] border dark:border-gray-800">
                  <div className="flex flex-col gap-3">
                    <span className="text-[10px] font-black uppercase tracking-widest mb-1 text-amber-500">
                      {t("problem_management.ai_generation_options") || "AI Generation Options"}
                    </span>
                    <Checkbox
                      size="sm"
                      isSelected={aiOptions.includeComplexity}
                      onValueChange={(val) => setAiOptions(prev => ({ ...prev, includeComplexity: val }))}
                    >
                      <span className="text-xs">{t("problem_management.ai_complexity") || "Complexity Analysis"}</span>
                    </Checkbox>
                    <Checkbox
                      size="sm"
                      isSelected={aiOptions.includeCorrectness}
                      onValueChange={(val) => setAiOptions(prev => ({ ...prev, includeCorrectness: val }))}
                    >
                      <span className="text-xs">{t("problem_management.ai_correctness") || "Correctness Proof"}</span>
                    </Checkbox>
                    <Checkbox
                      size="sm"
                      isSelected={aiOptions.includePseudocode}
                      onValueChange={(val) => setAiOptions(prev => ({ ...prev, includePseudocode: val }))}
                    >
                      <span className="text-xs">{t("problem_management.ai_pseudocode") || "Include Pseudocode"}</span>
                    </Checkbox>
                    <p className="text-[9px] text-gray-500 italic mt-2 border-t pt-2">
                      {t("problem_management.ai_generation_warning") || "Nếu gặp lỗi \"Incomplete\", hãy thử tắt bớt tùy chọn để giảm độ dài nội dung."}
                    </p>
                  </div>
                </PopoverContent>
              </Popover>
